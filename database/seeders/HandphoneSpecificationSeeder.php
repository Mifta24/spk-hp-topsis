        ...
        foreach ($jsonData['RECORDS'] as $record) {
            if (!empty($record['name']) && strpos($handphoneName, $record['name']) !== false) {
                return [
                    ...
                    'os_version' => $record['specifications']['operating_system'] ?? '-',
                    'network' => $record['Technology'] ?? '-',
                    'sim' => $record['SIM'] ?? '-',
                    'weight' => $record['Weight'] ?? '-',
                    'dimensions' => $record['specifications']['dimensions'] ?? '-',
                    'colors' => json_encode($record['spefications']['colors'] ?? '-'),
                    'features' => json_encode($record['Features'] ?? '-'),
